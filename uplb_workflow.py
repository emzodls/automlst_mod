...
def startjob(indir,resultdir,skip="",checkpoint=False,workflow=1,refdb="",cpu=1,concat=False,
    ...
    if workflow == 1:
        ...
        log.info("Diverting Workflow")
        try:
            return startUPLBwf(indir, resultdir, checkpoint=checkpoint, skip=skip, refdb=refdb, cpu=cpu, concat=concat,
                            model=model, bs=bs, kf=kf, maxmlst=maxmlst, maxorg=maxorg, filtMLST=filtMLST, fast=fast,
                            minmlst=minmlst)
        except Exception as e:
            log.error("Unexpected failure: %s"%e)
            raise
...
def startUPLBwf(indir,resultdir,checkpoint=False,concat=False,mashmxdist=0.5,cpu=1,
             skip="",refdb="",hmmdb="",rnadb="",maxmlst=100,model="GTR",bs=0,
             kf=False,maxorg=50,filtMLST=True,fast=False,minmlst=10):
    """WORKFLOW 1: Build phylogeny from scratch"""
    if not checkpoint:
    ...
    if checkpoint == "w1-1":
        for idx,queryseq in enumerate(queryseqs):
            log.info("JOB_STATUS::Running MASH ANI estimation against reference sequences...{}".format(len(queryseqs)))
            mashresult = mash.getdistances([queryseq],resultdir,cpu=cpu,maxdist=mashmxdist)
            if mashresult:
        #        checkpoint = "w1-2"
                log.info("JOB_CHECKPOINT::%s"%checkpoint)
                log.info("JOB_PROGRESS::Seq {} of {}".format(idx+1,len(queryseqs)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-wf","--workflow", help="Workflow to use 1 or 2",choices=(1,2,3),type=int, default=1)
    ...
    parser.add_argument("-mo","--maxorg", help="Maximum number of organisms to use (default:50)",type=int, default=75)
