...
def getOrgs(jobid):
    if os.path.exists('/Users/labuser/Downloads/userlist.json'):
        with open('/Users/labuser/Downloads/reflist_test.json','r') as reffile, open('/Users/labuser/Downloads/userlist.json','r') as userfile:
            refdict = json.load(reffile)
            userdict = json.load(userfile)
            refdict.update(userdict)
            return jsonify(refdict)
    elif os.path.exists('/Users/labuser/Downloads/reflist_test.json'):
        return send_from_directory(app.config['RESULTS_FOLDER'], 'reflist_test.json')
